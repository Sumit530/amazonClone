...
router.post("/addproduct", async (req, res) => {
    console.log(req.body);
    const { id ,title,price,description,discount,tagline } = req.body;

    if (!id || !title || !price || !description || !discount || !tagline) {
        res.status(422).json({ error: "filll the all details" });
        console.log("sari detail nahi hai");
    };

    try {

        const predata = await products.findOne({id:id});

        if (predata) {
            res.status(422).json({ error: "This data is already exist" });
        } else {
...
router.get("/users", async (req, res) => {
    ...
    try {
        ...
        console.log(usersdata + "ind mila hai");
...
router.delete("/deluser/:id",async(req,res)=> {
    try{
        ...
        console.log("error hai delete api me " + error);
    }
})
...
router.delete("/delproduct/:id",async(req,res)=> {
    try{
        ...
        const data = await products.findByIdAndRemove({_id:id})
        const store = await data.save();
        res.status(200).json(store);
...
router.get("/deluser/:id", async (req, res) => {
    try {
        const deluser = await User.findByIdAndRemove({ _id: req.userID });
        console.log(validuserone + "user hain home k header main pr");
        res.status(201).json(validuserone);
    } catch (error) {
        console.log(error + "error for valid user");
